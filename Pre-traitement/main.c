...
void main () {
  ...
  int moyen;    // PRE-TRAITEMENT 1)passage en gris 2)noir ou blanc
  for(i=0;i<pix_y;i++) {
    for(j=0;j<pix_x;j++) {
        image[i][j][2] = fgetc(streamIn);  // use BMP 24bit with no alpha channel
        image[i][j][1] = fgetc(streamIn);  // BMP uses BGR but we want RGB, grab //byte-by-byte
        image[i][j][0] = fgetc(streamIn);
        moyen = filtre_gris(image[i][j][0],image[i][j][1],image[i][j][2]);
        moyen = white_black(moyen);
        grid_image[i][j] = moyen;
        if ((j+1)==pix_x) {
        // incorporating nextline ('\n') to ensure alignment for edge pixels
        fprintf(num_img,"%d\n",moyen); // print R component
      }
      else {
        // for inner pixels, nextline(\n) not required
        fprintf(num_img,"%d",moyen); // R component
      }
    }
  }
